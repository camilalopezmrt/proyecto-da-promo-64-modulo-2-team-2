...
/* 2.3 Combinaciones de géneros normalizadas: mayor popularidad y engagement */
...
WHERE combinacion_generos LIKE '%+%'
...
SELECT
	...
	ROUND(AVG(CASE WHEN MONTH(t.track_release_date) NOT IN (11,12) THEN t.playcount END)) AS media_reproducciones_resto_2022,
	MAX(CASE WHEN MONTH(t.track_release_date) IN (11,12) THEN t.playcount END) - ROUND(AVG(CASE WHEN MONTH(t.track_release_date) NOT IN (11,12) THEN t.playcount END)) AS diferencia_reproducciones,
	MAX(CASE WHEN MONTH(t.track_release_date) IN (11,12) THEN t.listeners END) AS peak_oyentes_mundial,
	ROUND(AVG(CASE WHEN MONTH(t.track_release_date) NOT IN (11,12) THEN t.listeners END)) AS media_oyentes_resto_2022,
	MAX(CASE WHEN MONTH(t.track_release_date) IN (11,12) THEN t.listeners END) - ROUND(AVG(CASE WHEN MONTH(t.track_release_date) NOT IN (11,12) THEN t.listeners END)) AS diferencia_oyentes
FROM track t
WHERE YEAR(t.track_release_date) = 2022
GROUP BY t.genre_extracted
ORDER BY diferencia_reproducciones DESC;
